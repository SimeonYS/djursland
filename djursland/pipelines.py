...
class DjurslandPipeline:

    # Database setup
    conn = sqlite3.connect('djursland.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `djursland`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM djursland WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO djursland (date, title, link, content)"
